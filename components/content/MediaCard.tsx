...
import { Languages, Play, Download, Heart, Star, Bookmark } from 'lucide-react'
...
export function MediaCard({
  ...
  // For series, play button directs to the series page to select an episode
      ...
      <div className="relative aspect-[2/3] block">
        <Link href={target} className="absolute inset-0 z-0" aria-label={title}>
          ...
          {image ? (
            ...
            <div className="flex h-full items-center justify-center px-4 text-center text-sm text-white/50 pointer-events-none">
              {title}
            </div>
          )}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-3 pointer-events-none">
            ...
            {translator ? (
              <p className="mt-1 flex items-center gap-1 text-[11px] font-medium leading-tight text-white/75">
