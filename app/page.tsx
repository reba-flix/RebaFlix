...
export default async function HomePage() {
  ...
  const mixedNewReleases = [...taggedNewReleases, ...taggedSeries].sort((a, b) => {
    const dateA = ('releaseDate' in a && a.releaseDate) ? new Date(a.releaseDate).getTime() : (a.createdAt ? new Date(a.createdAt).getTime() : 0)
    const dateB = ('releaseDate' in b && b.releaseDate) ? new Date(b.releaseDate).getTime() : (b.createdAt ? new Date(b.createdAt).getTime() : 0)
    return dateB - dateA
  })
